...
export class MasalaValidator {
    ...
    checkPersonAreGreetedAtMostOnce(model: Model, accept: ValidationAcceptor): void {
        ...
        this.checkUniqueEntityName(model, accept);
        this.checkUniqueAtomName(model, accept);
        this.checkUniqueMoleculeName(model, accept);
        this.checkUniqueOrganismName(model, accept);
        this.checkUniquePageName(model, accept);
    }
    ...
    checkUniqueAtomName(model: Model, accept: ValidationAcceptor): void {
        ...
        model.atoms.forEach(atom => {
            ...
            //if the counter is greater than 1, create a helpful error
            if(newValue > 1) {
                accept('error', `You can define atom at most once! This is the ${newValue}${newValue==2?'nd':'th'} greeting to ${atom.name}.`, {
                    node: atom
                });
            }
        });
    ...
    checkUniqueMoleculeName(model: Model, accept: ValidationAcceptor): void {
        ...
        model.molecules.forEach(molecule => {
            ...
            //if the counter is greater than 1, create a helpful error
            if(newValue > 1) {
                accept('error', `You can define molecule at most once! This is the ${newValue}${newValue==2?'nd':'th'} greeting to ${molecule.name}.`, {
                    node: molecule
                });
            }
        });
    ...
    checkUniqueOrganismName(model: Model, accept: ValidationAcceptor): void {
        ...
        model.organisms.forEach(organism => {
            ...
            //if the counter is greater than 1, create a helpful error
            if(newValue > 1) {
                accept('error', `You can define organism at most once! This is the ${newValue}${newValue==2?'nd':'th'} greeting to ${organism.name}.`, {
                    node: organism
                });
            }
        });
    ...
    checkUniquePageName(model: Model, accept: ValidationAcceptor): void {
        ...
        model.pages.forEach(page => {
            ...
            //if the counter is greater than 1, create a helpful error
            if(newValue > 1) {
                accept('error', `You can define page at most once! This is the ${newValue}${newValue==2?'nd':'th'} greeting to ${page.name}.`, {
                    node: page
                });
            }
        });
