 ...
 */
export type MasalaAddedServices = {
    validation: {
        MasalaValidator: MasalaValidator
    }
}
 ...
 */
export type MasalaServices = LangiumServices & MasalaAddedServices
 ...
 */
export function createMasalaServices(context: DefaultSharedModuleContext): {
    shared: LangiumSharedServices,
    Masala: MasalaServices
} {
    const shared = inject(
        createDefaultSharedModule(context),
        MasalaGeneratedSharedModule
    );
    const Masala = inject(
        createDefaultModule({ shared }),
        MasalaGeneratedModule,
        MasalaModule
    );
    shared.ServiceRegistry.register(Masala);
    registerValidationChecks(Masala);
    if (!context.connection) {
        // We don't run inside a language server
        // Therefore, initialize the configuration provider instantly
        shared.workspace.ConfigurationProvider.initialized({});
    }
    return { shared, Masala };
}
