...
import { DomainModelElementTypeNames } from "masala-language";
...
export const UserStoriesMindMapPreview = ({ data }: TreeProps) => {
  ...
  useEffect(() => {
    ...
    svg.call(
      zoom.transform,
      d3.zoomIdentity.translate(width / 20, height / 30).scale(0.5)
    );
    ...
    g.selectAll(".link")
      ...
      .attr("d", (d) => {
        ...
        return `M${d.source.y},${d.source.x}C${d.source.y + 100},${
          d.source.x
        } ${d.target.y - 100},${d.target.x} ${d.target.y},${d.target.x}`;
      })
      .style("fill", "none")
      .style("stroke", "white")
    ...
    node
      ...
      .attr("transform", (d) =>
        d.children
          ? `translate(${d.data.name.length * 5}, -20)`
          : "translate(5, 0)"
      )
      .style("font-size", "1em")
